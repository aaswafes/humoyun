...
import { Disclosure } from "@/components/watch/watch-fields";
import { videoRangeLabel } from "./youtube-plan";
 ...
 *
 * A single video has one row and no numbering — "1 of 1 videos" is a fact
 * nobody needs.
 */
export function VideoLog({ item, className }: { item: Media; className?: string }) {
  ...
  const summary = watched.length
    ? single
      ...
      : `${watched.length} ${watched.length === 1 ? "sitting" : "sittings"} · ${videos} watched`
  ...
  return (
    <Disclosure
      storageKey="humoyun.youtube.sheet.log"
      label="Log"
      summary={summary}
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate text-[12px] font-medium text-ink tnum">
                  {single ? "Watched" : videoRangeLabel(range)}
                </span>
                {t.duration_min ? (
