...
import type { SortDir } from "@/components/watch/media-table";
import type { VideoSortKey } from "./video-table";

export type YoutubeStatusFilter = "all" | "watching" | "planned" | "finished" | "paused";
export type YoutubeGroupBy =
  | "status" | "kind" | "channel" | "genre" | "topic" | "series" | "none";

const FILTERS: { value: YoutubeStatusFilter; label: string }[] = [
  { value: "all", label: "All" },
...
const GROUPS: { value: YoutubeGroupBy; label: string }[] = [
...
const SORTS: { value: VideoSortKey; label: string }[] = [
...
export function YoutubeToolbar({
  ...
  filter: YoutubeStatusFilter;
  onFilter: (v: YoutubeStatusFilter) => void;
  group: YoutubeGroupBy;
  onGroup: (v: YoutubeGroupBy) => void;
  sort: VideoSortKey;
  onSort: (v: VideoSortKey) => void;
  dir: SortDir;
  onDir: (v: SortDir) => void;
  /** the table sorts from its own column headers, so grouping is shelf-only */
  showGrouping: boolean;
  count: number;
  /** everything on the shelf — the count only earns its place when it differs */
  total: number;
  className?: string;
}) {
  const inputId = React.useId();
  const sortLabel = SORTS.find((s) => s.value === sort)?.label ?? "Recently added";
    ...
    <div className={cn("flex flex-wrap items-center gap-2", className)}>
      <div className="relative min-w-[180px] flex-1 sm:max-w-[260px]">
        ...
        <input
          ...
          aria-label="Search videos and playlists by title, channel, genre, topic or series"
          placeholder="Search title, channel, genre, topic"
          className={cn(
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        {/*
          ...
          a page whose job is to show what is queued up. One quiet trigger that
          names the current sort holds all three.
        */}
        <Popover
          align="end"
